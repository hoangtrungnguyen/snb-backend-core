"""
Tests for POST /auth/owner/forgot-password endpoint.
...
class OwnerForgotPasswordViewTests(TestCase):
    """Tests for the POST /auth/owner/forgot-password endpoint."""

    def setUp(self):
        self.client = Client()
        self.url = "/auth/owner/forgot-password"
    ...
    def test_calls_supabase_recover_endpoint_with_email(self):
        """View must call POST /auth/v1/recover with the provided email."""
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {}
