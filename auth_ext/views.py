...
class OwnerSignupView(View):
    ...
    Creates an auto-confirmed Supabase auth user via the admin API
    (``email_confirm=True``, so no confirmation email is sent and the account
    is usable immediately) with ``app_metadata.role="owner"`` stamped on the
    user (so issued JWTs carry the role the auth middleware reads), then
    promotes the trigger-inserted `customers` row from ``player`` to ``owner``.
    ...
    def post(self, request):
        ...
        # Create an auto-confirmed user via the admin API. email_confirm=True
        # marks the email verified at creation, so no confirmation email is sent
        # and the owner can log in immediately. app_metadata.role="owner" is
        # stamped on the user so every issued JWT carries the role the auth
        # middleware reads.
        ...
        # from the default role="player" to "owner".
        if user_id:
            try:
